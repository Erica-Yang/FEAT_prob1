...
class FEAT(FewShotModel):
    ...
    def _forward(self, instance_embs, support_idx, query_idx):
        ...
        support = instance_embs[support_idx.contiguous().view(-1)].contiguous().view(
            *(support_idx.shape + (-1,)))  # [1,1,5,-1]->[1,1,5,640]   #取instance_embs前5个
        query = instance_embs[query_idx.contiguous().view(-1)].contiguous().view(
            *(query_idx.shape + (-1,)))  # [1,15,5,640] 取instance_embs的5~80（对1-shot)个

        # get mean of the support
        proto = support.mean(dim=1)  # Ntask x NK x d  [1,1,5,640] 试试5-way 5-shot [1,5,5,640]-->平均后[1,5,640]
